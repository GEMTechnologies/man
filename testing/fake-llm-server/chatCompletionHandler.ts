...
<man-write path="new/image/file.png" description="Uploaded image to codebase">
MAN_ATTACHMENT_0
</man-write>
`;
      messageContent += "\n\n" + generateDump(req);
    ...
    if (
      ...
      messageContent = `
<man-write path="src/bad-file.ts" description="Fix 2 errors and introduce a new error.">
...
x.nonExistentMethod2();
    ...
    if (
      ...
      // Fix errors in create-ts-errors.md and introduce a new error
      messageContent = `
<man-write path="src/bad-file.ts" description="Fix remaining error.">
// Import doesn't exist
// import NonExistentClass from 'non-existent-class';
...
x.toString(); // replaced with existing method
    ...
    if (
      ...
      messageContent = `
      Fixing the error...
      <man-write path="src/pages/Index.tsx">
      

import { MadeWithMan } from "@/components/made-with-man";
...
export default Index;
    ...
    if (
      ...
      messageContent = `[[STRING_IS_FINISHED]]";</man-write>\nFinished writing file.`;
      messageContent += "\n\n" + generateDump(req);
    }
    const isToolCall = !!(
      lastMessage &&
...
function generateDump(req: Request) {
  ...
  try {
    ...
    return `[[man-dump-path=${dumpFilePath}]]`;
  } catch (error) {
