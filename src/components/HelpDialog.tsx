...
export function HelpDialog({ isOpen, onClose }: HelpDialogProps) {
  ...
  const isManProUser = settings?.providerSettings?.["auto"]?.apiKey?.value;
...
## System Information
- man Version: ${debugInfo.manVersion}
...
`;
      ...
      const githubIssueUrl = `https://github.com/man-sh/man/issues/new?title=${encodedTitle}&labels=bug,filed-from-app&body=${encodedBody}`;
      ...
      IpcClient.getInstance().openExternalUrl(
        "https://github.com/man-sh/man/issues/new",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmitChatLogs = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://upload-logs.man.sh/generate-upload-url",
        {
          method: "POST",
          headers: {
...
`;
    ...
    const githubIssueUrl = `https://github.com/man-sh/man/issues/new?title=${encodedTitle}&labels=support&body=${encodedBody}`;
          ...
          <div className="space-y-4 overflow-y-auto flex-grow">
            ...
            <div className="border rounded-md p-3">
              ...
              <div className="text-sm bg-slate-50 dark:bg-slate-900 rounded p-2 max-h-32 overflow-y-auto">
                <p>man Version: {chatLogsData.debugInfo.manVersion}</p>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Need help with man?</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-col space-y-4 w-full">
          {isManProUser ? (
            <div className="flex flex-col space-y-2">
              ...
              >
                <SparklesIcon className="mr-2 h-5 w-5" /> Chat with man help
                bot (Pro)
              </Button>
              <p className="text-sm text-muted-foreground px-2">
                Opens an in-app help chat assistant that searches through man's
                docs.
              </p>
            </div>
          ) : (
