import { ismanProEnabled, type LargeLanguageModel } from "@/lib/schemas";
...
export function ModelPicker() {
  ...
  const isSmartAutoEnabled =
    settings.enableProSmartFilesContextMode && ismanProEnabled(settings);
  ...
  if (settings && ismanProEnabled(settings)) {
          ...
          <>
            ...
            {primaryProviders.map(([providerId, models]) => {
              models = models.filter((model) => {
                // Don't show free models if man Pro is enabled because
                // we will use the paid models (in man Pro backend) which
