...
export async function processFullResponseActions(
  ...
  try {
    // Extract all tags
    const manWriteTags = getManWriteTags(fullResponse);
    const manRenameTags = getManRenameTags(fullResponse);
    const manDeletePaths = getManDeleteTags(fullResponse);
    const manAddDependencyPackages = getManAddDependencyTags(fullResponse);
    const manExecuteSqlQueries = chatWithApp.app.supabaseProjectId
    ...
    if (manExecuteSqlQueries.length > 0) {
      for (const query of manExecuteSqlQueries) {
        try {
      ...
      logger.log(`Executed ${manExecuteSqlQueries.length} SQL queries`);
    }

    // TODO: Handle add dependency tags
    if (manAddDependencyPackages.length > 0) {
      try {
        await executeAddDependency({
          packages: manAddDependencyPackages,
          message: message,
          appPath,
        });
      } catch (error) {
        errors.push({
          message: `Failed to add dependencies: ${manAddDependencyPackages.join(
            ", ",
          )}`,
          error: error,
        });
    ...
    for (const filePath of manDeletePaths) {
    ...
    for (const tag of manRenameTags) {
    ...
    hasChanges =
      ...
      manAddDependencyPackages.length > 0;
    ...
    if (hasChanges) {
      ...
      if (manAddDependencyPackages.length > 0)
        changes.push(
          `added ${manAddDependencyPackages.join(", ")} package(s)`,
        );
      if (manExecuteSqlQueries.length > 0)
        changes.push(`executed ${manExecuteSqlQueries.length} SQL queries`);

      let message = chatSummary
        ? `[man] ${chatSummary} - ${changes.join(", ")}`
        : `[man] ${changes.join(", ")}`;
      ...
      if (uncommittedFiles.length > 0) {
        ...
        try {
          ...
          logger.log(
            `Amend commit with changes outside of man: ${uncommittedFiles.join(", ")}`,
          );
        } catch (error) {
          // Just log, but don't throw an error because the user can still
          // commit these changes outside of man if needed.
          logger.error(
            `Failed to commit changes outside of man: ${uncommittedFiles.join(
              ", ",
            )}`,
          );
    ...
    ${warnings
      .map(
        (warning) =>
          `<man-output type="warning" message="${warning.message}">${warning.error}</man-output>`,
      )
      .join("\n")}
    ${errors
      .map(
        (error) =>
          `<man-output type="error" message="${error.message}">${error.error}</man-output>`,
      )
      .join("\n")}
    `;
