...
const TEST_RESPONSES: Record<string, string> = {
  ...
  <man-add-dependency packages="deno"></man-add-dependency>
  
  EOM`,
  "add-non-existing-dep": `I'll add that dependency for you.
  
  <man-add-dependency packages="@angular/does-not-exist"></man-add-dependency>
  
  EOM`,
  "add-multiple-deps": `I'll add that dependency for you.
  
  <man-add-dependency packages="react-router-dom react-query"></man-add-dependency>
  
  EOM`,
  write: `Hello world
  <man-write path="src/hello.ts" content="Hello world">
...
export function getTestResponse(prompt: string): string | null {
  const match = prompt.match(/\[man-qa=([^\]]+)\]/);
  if (match) {
