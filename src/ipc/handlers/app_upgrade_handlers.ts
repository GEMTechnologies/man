...
const availableUpgrades: Omit<AppUpgrade, "isNeeded">[] = [
  {
    ...
    manualUpgradeUrl: "https://man.sh/docs/upgrades/select-component",
  },
  {
    id: "capacitor",
    ...
    manualUpgradeUrl: "https://man.sh/docs/guides/mobile-app#upgrade-your-app",
  },
];
...
function isComponentTaggerUpgradeNeeded(appPath: string): boolean {
  ...
  try {
    ...
    return !viteConfigContent.includes("@man-sh/react-vite-component-tagger");
  } catch (e) {
...
async function applyComponentTagger(appPath: string) {
  ...
  if (
    !content.includes(
      "import manComponentTagger from '@man-sh/react-vite-component-tagger';",
    )
  ) {
  ...
  if (content.includes("plugins: [")) {
    if (!content.includes("manComponentTagger()")) {
      content = content.replace(
        "plugins: [",
        "plugins: [manComponentTagger(), ",
      );
    }
  } else {
    throw new Error(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const process = spawn(
      "pnpm add -D @man-sh/react-vite-component-tagger || npm install --save-dev --legacy-peer-deps @man-sh/react-vite-component-tagger",
      {
        cwd: appPath,
        shell: true,
        stdio: "pipe",
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[man] add man component tagger",
    });
...
async function applyCapacitor({
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[man] add Capacitor for mobile app support",
    });
