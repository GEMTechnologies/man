...
export class PageObject {
  ...
  async snapshotMessages({
    ...
    if (replaceDumpPath) {
      // Update page so that "[[man-dump-path=*]]" is replaced with a placeholder path
  ...
  async snapshotServerDump(
    ...
    const dumpPaths = dumpPathMatches
      .map((match) => {
        const pathMatch = match.match(/\[\[man-dump-path=([^\]]+)\]\]/);
    ...
    ).replaceAll(/\[\[man-dump-path=([^\]]+)\]\]/g, "[[man-dump-path=*]]");
  ...
  getAppPath({ appName }: { appName: string }) {
    return path.join(this.userDataDir, "man-apps", appName);
  }
...
export const test = base.extend<{
  ...
  po: [
    async ({ electronApp }, use) => {
      ...
      const po = new PageObject(electronApp, page, {
        userDataDir: (electronApp as any).$manUserDataDir,
      });
      await use(po);
    },
    { auto: true },
  ...
  electronApp: [
    async ({ electronConfig }, use) => {
      ...
      process.env.MAN_ENGINE_URL = "http://localhost:3500/engine/v1";
      process.env.MAN_GATEWAY_URL = "http://localhost:3500/gateway/v1";
      ...
      const userDataDir = path.join(baseTmpDir, `man-e2e-tests-${Date.now()}`);
      ...
      (electronApp as any).$manUserDataDir = userDataDir;
      ...
      if (os.platform() === "win32") {
        try {
          console.log("[cleanup:start] Killing man.exe");
          console.time("taskkill");
          execSync("taskkill /f /t /im man.exe");
          console.timeEnd("taskkill");
          console.log("[cleanup:end] Killed man.exe");
        } catch (error) {
          console.warn(
            "Failed to kill man.exe: (continuing with test cleanup)",
            error,
          );
        }
      } else {
    ...
    .map((message) => {
      const content = Array.isArray(message.content)
        ...
        : message.content
            ...
            .replace(
              /\n<man-file path="package\.json">[\s\S]*?<\/man-file>\n/g,
              "",
            );
      return `===\nrole: ${message.role}\nmessage: ${content}`;
    })
