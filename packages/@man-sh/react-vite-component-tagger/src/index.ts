 ...
 */
export default function manTagger(): Plugin {
  return {
    name: "vite-plugin-man-tagger",
    apply: "serve",
    enforce: "pre",
    ...
    async transform(code: string, id: string) {
      try {
        ...
        walk(ast as any, {
          enter(node: any) {
            try {
              ...
              // ── 2. Check whether the tag already has data-man-id ───────────────
              ...
              console.warn(
                `[man-tagger] Warning: Failed to process JSX node in ${id}:`,
                error,
              );
            }
          },
        ...
        console.warn(
          `[man-tagger] Warning: Failed to transform ${id}:`,
          error,
        );
        return null;
      }
