(() => {
  const OVERLAY_ID = "__man_overlay__";
  let overlay, label;
  ...
  function updateOverlay(el, isSelected = false) {
    ...
    const name = el.dataset.manName || "<unknown>";
    const file = (el.dataset.manId || "").split(":")[0];
  ...
  function onMouseMove(e) {
    ...
    let el = e.target;
    while (el && !el.dataset.manId) el = el.parentElement;
  ...
  function onClick(e) {
    ...
    window.parent.postMessage(
      {
        type: "man-component-selected",
        id: state.element.dataset.manId,
        name: state.element.dataset.manName,
      },
      "*",
    );
  }
  ...
  function onKeyDown(e) {
    ...
    if (key === "c" && hasShift && hasCtrlOrMeta) {
      ...
      window.parent.postMessage(
        {
          type: "man-select-component-shortcut",
        },
        "*",
      );
    }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (e.data.type === "activate-man-component-selector") activate();
    if (e.data.type === "deactivate-man-component-selector") deactivate();
  });
  ...
  function initializeComponentSelector() {
    if (!document.body) {
      console.error(
        "man component selector initialization failed: document.body not found.",
      );
      return;
    }
    setTimeout(() => {
      if (document.body.querySelector("[data-man-id]")) {
        window.parent.postMessage(
          {
            type: "man-component-selector-initialized",
          },
          "*",
        );
        console.debug("man component selector initialized");
      } else {
        console.warn(
          "man component selector not initialized because no DOM elements were tagged",
        );
      }
    }, 0);
  }
