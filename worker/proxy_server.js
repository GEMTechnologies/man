...
let manShimContent = null;
let manComponentSelectorClientContent = null;
try {
...
try {
  const manShimPath = path.join(__dirname, "man-shim.js");
  manShimContent = fs.readFileSync(manShimPath, "utf-8");
  parentPort?.postMessage("[proxy-worker] man-shim.js loaded.");
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read man-shim.js: ${error.message}`,
  );
}

try {
  const manComponentSelectorClientPath = path.join(
    __dirname,
    "man-component-selector-client.js",
  );
  manComponentSelectorClientContent = fs.readFileSync(
    manComponentSelectorClientPath,
    "utf-8",
  );
  parentPort?.postMessage(
    "[proxy-worker] man-component-selector-client.js loaded.",
  );
} catch (error) {
  parentPort?.postMessage(
    `[proxy-worker] Failed to read man-component-selector-client.js: ${error.message}`,
  );
}
...
function injectHTML(buf) {
  ...
  // These are strings that were used since the first version of the man shim.
  // If the man shim is used from legacy apps which came pre-baked with the shim
  ...
  if (!legacyAppWithShim) {
    ...
    if (manShimContent) {
      scripts.push(`<script>${manShimContent}</script>`);
    } else {
      scripts.push(
        '<script>console.warn("[proxy-worker] man shim was not injected.");</script>',
      );
    }
  }
  if (manComponentSelectorClientContent) {
    scripts.push(`<script>${manComponentSelectorClientContent}</script>`);
  } else {
    scripts.push(
      '<script>console.warn("[proxy-worker] man component selector client was not injected.");</script>',
    );
  }
