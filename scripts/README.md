...
1. `man-{version}-1.x86_64.rpm` (Linux RPM)
2. `man-{version}-full.nupkg` (Windows NuGet package)
3. `man-{version}.Setup.exe` (Windows installer)
4. `man-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `man-darwin-x64-{version}.zip` (macOS Intel)
6. `man_{version}_amd64.deb` (Linux DEB)
